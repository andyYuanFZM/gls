...
build: toolimport
	CGO_ENABLED=1 go build ${BUILD_FLAGS} -v  -o glass
	CGO_ENABLED=1 go build ${BUILD_FLAGS} -v  -o glass-cli github.com/andyYuanFZM/glass/cli
...
GOBUILD=CGO_ENABLED=1 go build $(BUILD_FLAGS)' -X "github.com/andyYuanFZM/glass/version.Version=$(VERSION)"  -w -s'
SRC_CLI := ./cli
SRC := ./
APP := bityuan
CLI := bityuan-cli
